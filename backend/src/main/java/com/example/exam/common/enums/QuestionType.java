...
public enum QuestionType {
     ...
     * 判分规则：选择正确选项得满分，否则0分
     */
    SINGLE_CHOICE("SINGLE_CHOICE", "单选题", "4个选项，仅1个正确"),

    /**
     * 多选题：4-5个选项，≥1个正确
     * 判分规则：选择与正确选项完全一致得满分；多选/少选/错选均0分
     */
    MULTIPLE_CHOICE("MULTIPLE_CHOICE", "多选题", "4-5个选项，至少1个正确"),

    /**
     * 不定项选择题：4个选项，≥1个正确，支持部分得分
     * 判分规则：选对部分正确选项按score_ratio累加得分；选错任何选项0分
     */
    INDEFINITE_CHOICE("INDEFINITE_CHOICE", "不定项选择题", "4个选项，支持部分得分"),

    /**
     * 判断题：2个选项（对/错），仅1个正确
     * 判分规则：选择正确选项得满分，否则0分
     */
    TRUE_FALSE("TRUE_FALSE", "判断题", "2个选项（对/错）"),

    /**
     * 匹配题：左侧题干选项、右侧匹配选项，一一对应
     * 判分规则：所有匹配正确得满分；每匹配正确1组得1/N分
     */
    MATCHING("MATCHING", "匹配题", "左侧题干与右侧选项一一对应"),

    /**
     * 排序题：3-5个选项，需按正确顺序排列
     * 判分规则：完全正确得满分；采用"逆序数法"计算偏差按比例扣分
     */
    SORT("SORT", "排序题", "3-5个选项，按正确顺序排列"),

    /**
     * 填空题：1-3个空，每个空有固定答案（支持多个正确答案）
     * 判分规则：每个空匹配任一正确答案即得分，支持模糊匹配
     */
    FILL_BLANK("FILL_BLANK", "填空题", "1-3个空，支持多个正确答案"),

    /**
     * 主观题：简答题/论述题，无固定选项，需人工批改
     * 判分规则：教师手动打分+批改评语
     */
    SUBJECTIVE("SUBJECTIVE", "主观题", "简答/论述题，需人工批改");
